...
echo "##### create flashback directory "fra1" #####"
cd ${DEST_NEW}
if [ ! -d "fra1" ];then
        cd ${DEST_NEW}
...
echo "#### Get original  archive path #####"
ARC_DEST=$(
...
echo "##### Set flashback point directory #####"
sqlplus -s / as sysdba <<EOF
set head off
...
echo "##### Closing the log application #####"
##关闭日志应用
sqlplus -s / as sysdba <<EOF
set head off
...
echo "##### Check the secondary DG application status #####"
#检查二级DG应用状态
for c in ${DEST_NAME[@]}
do
...
echo "##### Start to shutdown instance #####"
##停库
sqlplus -s / as sysdba <<EOF 
shutdown immediate;
startup mount;
...
echo "##### Create flashback point,convert database to snapshot #####"
##新建闪回点,打开到snapshot模式
sqlplus -s / as sysdba <<EOF
set head off
...
echo "##### Start to open the instance #####"
sqlplus -s / as sysdba <<EOF
set head off
alter database open; 
exit
...
echo "##### Start to shutdown instance,convert database to PHYSICAL,then Start to shutdown instance again #####"
sqlplus -s / as sysdba <<EOF
set head off
shutdown immediate;
startup mount;
...
echo "##### Drop flashback point,open database read-only #####"
sqlplus -s / as sysdba <<EOF
set head off
startup mount;
...
echo "##### Set original archive path #####"
...
echo "##### Check the log transmission channel #####"
##检查通道
DEST_NAME=$(sqlplus -s / as sysdba <<EOF
set head off
select DEST_NAME from v\$archive_dest where dest_name like 'LOG_ARCHIVE_DEST_%' and DESTINATION is not null and dest_name<>'LOG_ARCHIVE_DEST_1';
...
echo "##### Open the log transmission channel #####"
##打开传输通道
for i in ${DEST_NAME[@]}
  do
    DEST_NAME_NUM=$(echo $i |awk  -F '_' '{print $4}')
